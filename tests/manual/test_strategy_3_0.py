...
# Configuración de path para importar módulos src
...
def explore_object_structure(raw_artifacts):
    ...
    try:
        ...
        prod_params = globals_json["properties"]["globalParameters"]["default_parameters_per_environment"]["value"]["prod"]
    ...
    if found_item:
        ...
        print(f"   🔹 Origen Bronze: {brz_cfg.get('source_system')} -> {brz_cfg.get('source_table')}")
        ...
        if mapping:
            print(f"      Ejemplo: {mapping[0]['source_column_name']} -> {mapping[0]['target_data_type']}")
        ...
        if script_path:
            ...
            if code:
                ...
                if "opflag" in code:
                    print("   ⚠️  Análisis: El código contiene lógica de 'opflag' (Borrado lógico detectado).")
