...
def _clone_or_pull_repo(repo_name: str, destination: Path, github_token: str) -> None:
    ...
    try:
        if destination.exists() and (destination / ".git").exists():
            ...
            logger.info(
                f"Repository {repo_name} already exists at {destination}, pulling updates..."
            )
            ...
            if lock_file.exists():
                ...
                try:
                    ...
                    logger.warning(
                        f"Could not remove lock file (may be in use by another process): {e}"
                    )
            ...
            try:
                ...
                if "would be overwritten" in str(pull_error) or "local changes" in str(pull_error):
                    logger.warning(
                        f"Pull failed due to local changes, resetting to remote state..."
                    )
            ...
            logger.info(
                f"Directory {destination} exists but is not a git repository. Removing and cloning fresh..."
            )
            import shutil
            ...
            logger.info(
                f"Cloning repository {repo_name} (this may take several minutes for large repositories)..."
            )
            ...
            logger.info(
                f"Cloning repository {repo_name} to {destination} (this may take several minutes for large repositories)..."
            )
