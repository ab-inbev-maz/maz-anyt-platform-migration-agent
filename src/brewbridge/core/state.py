...
class MigrationGraphState(BaseModel):
    ...
    manifest_path: Optional[str] = Field(
        None, description="Path to the manifest.yaml file defining the migration batch."
    )
    credentials: Optional[Dict[str, str]] = Field(
        default_factory=dict, description="API credentials (GitHub, DataFactory, SQL, etc.)."
    )
    ...
    pipelines_to_migrate: List[Dict[str, Any]] = Field(
        default_factory=list, description="List of pipelines included in the migration manifest."
    )
    current_pipeline_data: Optional[Dict[str, Any]] = Field(
        None, description="Metadata of the currently processed pipeline from the manifest."
    )

    # --- SECTION 2: Framework Detection & Templates ---
    source_framework: Optional[str] = Field(
        None, description="Identified origin framework (e.g., '3.0', 'cobos')."
    )
    environment_type: Optional[str] = Field(
        None, description="Environment type derived from pipeline name (brz, slv, gld)."
    )
    ...
    transform_template: Optional[str] = Field(
        None, description="Raw text of the transformations template for Hopsflow."
    )
    notebook_template: Optional[str] = Field(
        None, description="Raw text of the Brewtiful notebook template (used for GLD)."
    )
    ...
    normalized_schema_v4: Optional[Dict[str, Any]] = Field(
        None, description="Normalized JSON schema serving as the canonical input for translators."
    )
    ...
    validator_output: Optional[str] = Field(
        None, description="Raw stdout/stderr from engineeringstore validation CLI."
    )
    validation_passes: bool = Field(
        default=False, description="Flag indicating whether DAG validation passed successfully."
    )
    retry_count: int = Field(
        default=0, description="Number of correction retries performed during validation loop."
    )

    # --- SECTION 7: Human-in-the-Loop Approval ---
    human_approval_decision: Optional[str] = Field(
        None, description="Manual decision provided by human reviewer (APPROVE or REJECT)."
    )

    # --- SECTION 8: Reporting & Deployment ---
    migration_summary_md: Optional[str] = Field(
        None, description="Markdown summary generated by ReporterLogger agent."
    )
    push_status: Optional[str] = Field(
        None, description="Result of the final git push to the 4.0 repository."
    )
